...
<body id="myPage">
    ...
    <div class="w3-container" style="margin-top: 80px;left: 2%;position:relative">
        <div style="display:inline-block">
            <p style="font-size: 28px;font-weight: bold;">รายชื่อพบ</p>
        </div>
    </div>
        ...
        <div class="w3-container">
            <div class="w3-row">
                <table class="w3-table w3-centered w3-bordered w3-border" style="width: 80%;margin: auto;">
                    ...
                    $sql = "SELECT member.firstname as mname,organization.firstname as oname,member.lastname as mlname,organization.lastname as olname,roles FROM account left outer join member on account.accountID = member.accountID left outer join organization on account.accountID = organization.accountID where member.firstname like '%" . $_POST["keyword"] . "%' or organization.firstname like '%" . $_POST["keyword"] . "%'";
                    $rs = $conn->query($sql);
                    if ($rs->num_rows != 0) { //Check that it's have in DB or not
                        while ($row = $rs->fetch_assoc()) {
                            if ($row['oname'] == null) {
                                $firstname = $row['mname'];
                            } elseif ($row['mname'] == null) {
                                $firstname = $row['oname'];
                            }
                            if ($row['olname'] == null) {
                                $lastname = $row['mlname'];
                            } elseif ($row['mlname'] == null) {
                                $lastname = $row['olname'];
                            }
                    ?>
                            <tr>
